...
@testset "Effort tests" begin
    ...
    @test isapprox(grad(central_fdm(5,1), x->D_z_x(z, x), x)[1], ForwardDiff.gradient(x->D_z_x(z, x), x), rtol=1e-4)
    ...
    @test isapprox((Effort._D_z_unnorm(1., 0.14/0.67^2, 0.67; mν =0.4, w0=-1.9, wa=0.7)/Effort._D_z_unnorm(0., 0.14/0.67^2, 0.67; mν =0.4, w0=-1.9, wa=0.7)), 0.5713231772620894, rtol=4e-5)
    D, f = Effort._D_f_z(z, Ωcb0, h; mν =mν, w0=w0, wa=wa)
    @test isapprox(D, Effort._D_z_unnorm(z, Ωcb0, h; mν =mν, w0=w0, wa=wa))
